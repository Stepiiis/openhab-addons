...
class EnergyBalancingEngineTest {
    ...
    EnergyManagerConfiguration config = new EnergyManagerConfiguration(new BigDecimal(30), new BigDecimal(MAX_PRODUCTION_POWER),
    ...
    void testEvaluateEnergy() {
        // setup
        var stateBuilder = InputItemsState.builder()
                .productionPower(new DecimalType(1000))
                .gridPower(new DecimalType(0))
                .storageSoc(new DecimalType(50))
                .storagePower(new DecimalType(-1000))
                .minStorageSoc(new DecimalType(50))
                .maxStorageSoc(new DecimalType(95))
                .electricityPrice(new DecimalType(0));
        doReturn(stateBuilder
                .build())
                .when(energyBalancingEngine).buildEnergyState(any());
        var outputChannels = new ArrayList<>(List.of(
                Map.entry(new ChannelUID(channeluidprefix + "prio1"), SurplusOutputParameters.builder()
                        .loadPower(1000)
                        .switchingPower(300)
                        .priority(1)
                        .build()
                ),
                Map.entry(new ChannelUID(channeluidprefix + "prio2"), SurplusOutputParameters.builder()
                        .loadPower(2000)
                        .switchingPower(1000)
                        .priority(2)
                        .build()
        ...
        doReturn(stateBuilder
                .storagePower(new DecimalType(-2000))
                .build())
                .when(energyBalancingEngine).buildEnergyState(any());
        ...
        // setup subsequent call
        doReturn(stateBuilder
                .storagePower(new DecimalType(0))
                .build())
                .when(energyBalancingEngine).buildEnergyState(any());
        doReturn(OnOffType.ON).when(stateHolder).getState(eq(new ChannelUID(channeluidprefix + "prio1")));
        ...
        // setup subsequent call
        doReturn(stateBuilder
                .gridPower(new DecimalType(1000))
                .storagePower(new DecimalType(0))
                .build())
                .when(energyBalancingEngine).buildEnergyState(any());
        doReturn(OnOffType.ON).when(stateHolder).getState(eq(new ChannelUID(channeluidprefix + "prio2")));
    ...
    void testAvailablePowerHeuristicDueToInverterLimiting() {
        // setup
        var stateBuilder = InputItemsState.builder()
                .productionPower(new DecimalType(3000))
                .gridPower(new DecimalType(0))
                .storageSoc(new DecimalType(95))
                .storagePower(new DecimalType(0))
                .minStorageSoc(new DecimalType(50))
                .maxStorageSoc(new DecimalType(95))
                .electricityPrice(new DecimalType(0));
        ...
        // verify that prio1 is turned on because the available power has been calculated based on maximum power available
        verify(updateStateFunction, times(1)).accept(eq(new ChannelUID(channeluidprefix + "prio1")), eq(OnOffType.ON));
        verify(updateStateFunction, never()).accept(eq(new ChannelUID(channeluidprefix + "prio1")), eq(OnOffType.OFF));
        verify(updateStateFunction, never()).accept(eq(new ChannelUID(channeluidprefix + "prio2")), eq(OnOffType.ON));
        verify(updateStateFunction, never()).accept(eq(new ChannelUID(channeluidprefix + "prio2")), eq(OnOffType.OFF));
        ...
        // setup subsequent call
        doReturn(stateBuilder
                .productionPower(new DecimalType(5000))
                .storagePower(new DecimalType(2000))
                .build())
                .when(energyBalancingEngine).buildEnergyState(any());
        doReturn(OnOffType.OFF).when(stateHolder).getState(eq(new ChannelUID(channeluidprefix + "prio2")));
        ...
        // verify it is turned off in the next call due to overestimation and the draw from battery equals to its deactivation power
        verify(updateStateFunction, times(1)).accept(eq(new ChannelUID(channeluidprefix + "prio1")), eq(OnOffType.ON));
        verify(updateStateFunction, times(1)).accept(eq(new ChannelUID(channeluidprefix + "prio1")), eq(OnOffType.OFF));
        verify(updateStateFunction, never()).accept(eq(new ChannelUID(channeluidprefix + "prio2")), eq(OnOffType.ON));
        verify(updateStateFunction, never()).accept(eq(new ChannelUID(channeluidprefix + "prio2")), eq(OnOffType.OFF));
